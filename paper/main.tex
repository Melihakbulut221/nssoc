...
% see the limitations section, which is where the observed answer is
% stated rather than the configured one.
%
...
On 11 September 2026, four defects were repaired in the system-on-chip's
accelerator and watchdog sources: an unbounded state in the event
engine's decision state from which it could not drain the die's queue, an
asynchronous board strap sampled into three triplicated replicas with no
synchroniser, a slave that advertised itself free while it still owed the
fabric a response, and three counter comparisons written as equality
where the file's own rule is inequality. All four are real, all four are
repaired, and each carries a test that fails on the unrepaired source.
