...
detailed-route DRC 0. \emph{Superseded 14 September 2026, and left
standing: that is the six-macro layout this paper measures throughout.
The design's default build carries eight macros and is worse, by more
than the remedies of \S\ref{sec:placement} recovered.
\texttt{s83romecc5} measures $-$\SI{7.5758}{\nano\second} on
\num{3529} endpoints and \texttt{s83ant}
$-$\SI{7.3492}{\nano\second} on \num{3550}, against
$-$\SI{4.6799}{\nano\second} on \num{3088} here---and hold, met at
all three corners in every layout this paper reports, is LOST in
\texttt{s83ant}: \num{55} violations at $-$\SI{0.4351}{\nano\second}.
The hold loss is the price of repairing that layout's last two antenna
violations, it is structural rather than a tuning artefact---a gentler
repair setting produces a byte-identical layout---and no number in this
paper has been re-measured on either build}
\\
...
\paragraph{The nearest neighbour is the author's own, and it is not
neuromorphic.} ZIRH-2~\cite{zirh2_2026} is a rad-hard experiment chip on
this same open PDK, public under Apache-2.0 since August 2026: a
triplicated SERV RISC-V, TMR chains placed in two deliberately different
arrangements, and SECDED RAM with scrub-on-read, carried through an open
flow to a routed layout. That is every property this design holds
\emph{except} the spiking accelerator. So the conjunction ``open plus
open-PDK plus triplicated plus error-coded plus routed'' is not what is
new here---it was already occupied, by us, on the same process. What
distinguishes the present work is not the artefact but the question
asked of it: ZIRH-2 places its replicas apart \emph{by construction} and
does not measure whether the toolchain preserved the structure it was
given, which is exactly the measurement
Sections~\ref{sec:placement} and~\ref{sec:decks} report.
