...
2075.22$\,\si{\um} carrying eight vendor memory macros and
...
\item \textbf{A registered request phase in the fabric.} The
      ...
      number in \cref{tab:headline}, and it is now priced: \dcite{84}{}
      takes the design's median combinational depth from 39 levels to 20
      and its maximum from 81 to 69, for \qty{24.89}{\percent} of the
      workload's cycle count and \qty{1.08}{\percent} of its area, and
      the layout of it stops at global routing with 387 overflowing
      tiles, 353 of them on one metal layer. The default stays off for
      that reason rather than for want of a price.
\item \textbf{Hold margin as a first-class budget.} Two separate
...
\item \textbf{The remaining peripherals.} A second serial port, the
      ...
      and bus codecs are specified and unbuilt; two of them --- the
      two-wire interface and the bus codec, both Wishbone-classic
      cores with an 8-bit data path --- are now unblocked by the one
      bridge that exists and is proved, and none of them is on the
