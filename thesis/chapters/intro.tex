...
inference in small-satellite missions, implemented on a 130\,nm
technology with an open-source RTL-to-GDS flow
(\code{README.md}; \dcite{00-index}{section 1}). Its architecture class
follows the Frontgrade Gaisler GR801 as published in that device's April
2026 product brief --- a RISC-V management processor, an event-driven
neuromorphic inference engine, on-chip SRAM and a set of spacecraft
interfaces --- retargeted from 28\,nm FDSOI to 130\,nm at a scale an open
...
institutional ecosystem; the third is that the open PDK ships foundry
compiled SRAM macros with a built-in BIST interface rather than
compiler-generated cells (\code{docs/04} section 3); the fourth is the
cost argument of the paragraph above; and the fifth is that the fallback
is unusually strong, because the flow is already silicon-proven on the
fallback node, so falling back costs a retarget rather than a flow
rebuild (\textbf{[estimate]} at 3--6 weeks, \dcite{04}{section 2}).
...
commands. Ten RTL blobs by git hash, the submission tree by a 31-file
manifest whose own SHA-256 is recorded, six flow configurations pinned
...
qualification body; the fault-injection results are simulated single-bit
upsets, not measured event rates; the timing, power and area figures are
pre-silicon flow results on layouts that are explicitly named where they
...
\textbf{The verification chapter} covers the four instruments and what
each can and cannot see: the bit-exact golden models that are the
specification, the cocotb regressions, the SymbiYosys property sets, the
riscv-formal bring-up against the ISA, and the seeded fault-injection
campaigns at register-transfer and gate level
