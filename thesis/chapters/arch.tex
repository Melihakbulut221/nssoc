...
\begin{figure}[tbp]
  ...
  eight vendor memory macros outlined. Only about seven thousand of the
  die's 168,584 instances still carry a hierarchical name after
  synthesis, so the rest are attributed by connectivity --- nets keep
...
one switch, and it was priced before it was thrown
...
core and costs almost nothing in frequency, 0.27\,ns, because the shadow
core is parallel rather than in series.

Area is not what decided it. Lockstep \emph{detects} where this project's voters \emph{mask}; the two
...
target, one gate equivalent $=$ 7.2576\,$\mu$m$^2$ --- the fabric is 868
...
\begin{sloppypar}
\Cref{tab:arch-memmap} is the map, and the addresses are the reference
family's, taken unchanged from \dcite{08}{section 3.1}.  The RAM is \textbf{32\,KiB,
...
byte-writable 64\,KiB build the PDK admits (the denser \code{8192x32} part has no byte-mask port, and Ibex emits
...
errors and a die that does not grow by one database unit; the superseded
estimate that it would have to grow by about 200\,$\mu$m is kept rather
than deleted, and it measured the wrong axis.
\end{sloppypar}
...
or into the space a narrower word leaves. \dcite{67}{section 3} costs six options from the vendor LEF; the cheapest
that keeps 64\,KiB is a seventh \code{2048x64} at
$+491{,}633.62\,\mu$m$^2$, $+21.88$\,\% of the macro area, with a
read-modify-write on every \code{sb} and \code{sh}. That is a design
...
(72,64) Hsiao code with data bits 8--63 tied to zero; three of the eight
...
\textbf{the recovery is a trap and not a wrong word}. A scrubber sits
...
Routed, the read-return group's worst slack falls from $-2.7112$ to
...
costs $+1{,}421.92\,\mu$m$^2$, $+0.52$\,\% of the core, and buys a
...
sources on one line, so a fourteenth interrupt-bearing peripheral is a
build failure naming the decision.
...
eight numbered properties in its own file header, against which the
formal properties and the cocotb suite are written
(\dcite{40}{section 5}, \dcite{60}{section 5.6}).
...
stores hold the machine in stage 1 for ever. \textbf{W6:} the 22 bits of
persistent, silent state are bundled into one word, tripled and voted
every cycle, while 36 flip-flops the block rewrites every tick or kick
are left alone. \textbf{W7 and W8}, a windowed kick mode and a kick
budget, are both measured; W7 is \textbf{recommended for removal}.
...
redundancy itself was counted rather than assumed --- 102 mapped
flip-flops, after stripping every protective attribute from the source
text --- at $+5{,}091.06\,\mu$m$^2$ and \textbf{1.6\,\% of what lockstep
would have cost}.
...
\begin{table}[tbp]\small
\centering
\caption{The peripheral slots, from \code{regmap/memmap.yaml}. Areas are the same recipe as every other block here, from different
sessions; a block re-measured in a different source list moves by of
order one per cent (\dcite{45}{section 4.3}).}
...
\code{0x008} & \code{0xFF908000} & TIMER0 and watchdog & scaler, config, 3 timers, \code{WDOGSTAT} & 8 & 3 & 33{,}528.07 \\
...
\code{0x017} & \code{0xFF917000} & BOOTREG & 4: straps, status, report, epoch & --- & --- & 7{,}229.06 \\
...
\textbf{65.051\,kGE, 171\,\% of the Ibex core}, about four fifths of it
FIFO storage at depths fixed in a generated package
(\dcite{65}{section 8}). What was written instead is
...
clocks on a core where one extra cycle of memory latency was measured at
25.23\,\% of run time (\dcite{66}{section 2.1}). The controller is checked against a behavioural \code{W25Q128JV} written
...
the map leaves above the reset vector. Three questions were open --- how the ROM gets contents, who writes the
RAM's check bits, and what happens when boot fails --- and they are one
...
ships no ROM generator, so the two \code{1024x32} macros are a
...
flip-flops \textbf{eighteen are protected} --- the boot counter, the
sampled straps, the watchdog-pin readback, three flags and the
system-reset sample --- bundled into one word and tripled under the
...
consequence. The campaign moved one bit into the protected set: the
...
(\dcite{70}{}) it costs \textbf{no die area and no hold margin}.
...
go to one slave port and one waits (\dcite{68}{section 8.1}).

\section{Telemetry and discovery}

Two blocks exist because a SECDED register file corrects silently, and in
silicon a corrected upset is indistinguishable from no upset at all. \textbf{BUSSTAT} carries four saturating counters and four
...
no retention, no sleep state, no power domains, and a reserved address
where the clock-gate block would be. That is its one measured requirement gap, four orders of magnitude wide:
on the telemetry mission profile's duty cycle the part is 0.0044\,\%
busy and 100\,\% powered
(\dcite{60}{section 4.3}).
...
transport and not the CPU is the bottleneck. There is \textbf{no debug module and no JTAG}. And there are
...
the eight vendor macros (\dcite{67}{section 5.1}). That row predates the
boot block's hardening; the netlist the placed design of record was built
from is a different configuration and carries \textbf{5{,}873
flip-flops} (\dcite{69}{section 7.4}, \dcite{60}{section 9.10.1}).
