...
a function left to take (\dcite{30}{section 3.3}; \dcite{41}{section 4.1}). Five of the watchdog's eight protected fields are one bit wide ---
\code{hw/soc/rtl/soc\_wdog.v}'s field offsets and
\dcite{41}{section 3.2}'s own table, whose widths sum to the 22 bits
quoted below, while that document's section 4.2 says six --- and
replicating one naively yields a single flip-flop with a voter voting
it against itself --- while \textbf{every functional
...
\SI{5.539}{\milli\watt} waiting at the typical corner, but on
\code{full3}, a layout \dcite{57}{section 3.2} itself found does not
contain the accelerator; \dcite{76}{section 2} could not reproduce that
pair on the design that exists and replaced it with its own baseline,
so the figures the gates were measured against are
\SI{33.359}{\milli\watt} computing and \SI{18.745}{\milli\watt} waiting
on the ungated control, becoming \SI{45.199}{\milli\watt} and
\SI{8.287}{\milli\watt} with the gates in (\dcite{76}{section 8.3}).
\dcite{76} added two clock gates, and the interesting part is what gating a
...
paired counterfactuals. Every injection is one single-bit flip at a
seeded cycle inside a measured window, into one flip-flop everywhere
except the two memory rows: there the deposit is one bit
exclusive-ored into a data or check word of one memory, 1{,}000 draws
by region of the program's own link map (\dcite{67}{section 7.1}), and
\dcite{60}{section 9.10} is explicit that a memory word is not a
flip-flop of this netlist.}

and at line 793: \textbf{Every campaign is single-bit}, and every
campaign but the memories' is flip-flop only ---
\dcite{67}{section 7.1}'s 1{,}000 draws land in a data or check word of
one memory --- with no multi-bit strike and no transient in
combinational
\label{tab:campaigns}
