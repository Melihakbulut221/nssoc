...
other \num{162} lies within \SI{0.500}{\um} of a macro edge}, in
six discrete values and no others, with nothing in the open field of
standard cells and routing. The bound reproduces at
\SI{0.500}{\um} on a second layout and, run backwards, at
\SI{0.380}{\um} on a six-macro layout measured eleven days
...
comparison is \emph{weaker} than the one that fails, whose failure includes incompatible macro views and bus naming. It does
not establish transistor-level SRAM connectivity.

No silicon exists. Nothing here was fabricated or exposed to a beam.
Every number is a deck output on a layout.
\end{abstract}

\paragraph{Historical measurement scope (22 September 2026).}
The numerical results below describe the archived runs named in this
manuscript, not the current RTL or a manufactured device. Subsequent
interface and boot-ROM changes require their own physical verification.
Current product acceptance and unresolved requirements are recorded in
\href{https://github.com/Melihakbulut221/nssoc/blob/codex/complete-open-work/docs/92-product-acceptance.md}{the product acceptance register}.
The earlier abstract called the failing LVS a ``bus-delimiter naming
artefact rather than a connectivity error''. That conclusion is withdrawn:
the comparison does not establish transistor-level SRAM connectivity.
...
{\small\path{libs.tech/klayout/tech/drc/rule_decks/}}, of which \num{38}
...
Every marker names a cell inside the vendor array. The cell families are
row decoders ({\small\path{RM_IHPSG13_1P_ROWDEC*}}), column decoders
({\small\path{COLDEC*}}) and internal standard cells
({\small\path{RSC_IHPSG13_*}}). The
reported core's standard cells, routing and power grid produce no
markers in that classification. This is not a pad-ring verification.
...
than \SI{10}{\um}, so ordinary routing beside it violates a
...
\midrule
\SI{0}{\um} (touching) & 2  & 2  & 4  \\
\SI{0.220}{\um}        & 31 & 31 & 1  \\
\SI{0.260}{\um}        & 39 & 55 & 27 \\
\SI{0.280}{\um}        & 4  & 5  & 21 \\
\SI{0.320}{\um}        & 54 & 54 & 51 \\
\SI{0.380}{\um}        & 15 & 12 & 3  \\
\SI{0.500}{\um}        & 5  & 3  & 0  \\
\midrule
total outside                   & \num{150} & \num{162} & \num{107} \\
\textbf{maximum}                & \textbf{\SI{0.500}{\um}}
                                & \textbf{\SI{0.500}{\um}}
                                & \textbf{\SI{0.380}{\um}} \\
\bottomrule
\end{tabular}
...
a distance. Its maximum is \SI{0.380}{\um}. The property was
...
\SI{0.225}{\um} well overhang---and it should be read with care.
Of the \num{33} boxes it reclassifies as straddling an edge, \num{31}
overlap by exactly \SI{0.005}{\um}, at three different macro
edges. The markers sit \SI{0.220}{\um} from the LEF edge and the
\SI{0.225}{\um} model overshoots them by five nanometres. Set
the constant to \SI{0.220}{\um} and all \num{31} move. That
...
instances of the following message:
\begin{quote}\small\ttfamily
[DRT-0349] LEF58\_ENCLOSURE with no CUTCLASS is not supported.\\
Skipping for layer X
\end{quote}
The layers are \texttt{Cont}, \texttt{Via1} through
...
layout nets, and a top-level pin-matching failure. One observed mismatch
is naming. The CDL spells buses \texttt{A\_DIN<32>} and globals
\texttt{VSS!}, \texttt{VDD!} where the layout has \texttt{A\_DIN[32]},
\texttt{VSS}, \texttt{VDD}. These differences do not establish that
normalising names alone would make the transistor-level comparison pass.
An abstract layout and a transistor-level schematic are different views;
macro-interior LVS remains unverified.
...
{\small\path{scripts/classify_magic_drc.py}}. It reproduces a previously
