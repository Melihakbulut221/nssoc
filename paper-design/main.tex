...
flip-flops and \SI{31391}{\um\squared}---\SI{4.78}{\percent} of
...
system-memory protection & \num{2068} & \num{234} &
  \SI{31391}{\um\squared} (\SI{4.78}{\percent}) \\
\bottomrule
\end{tabular}
\end{center}
...
The early whole-SoC synthesis measurement gives \num{27694} cells
and \num{3077} flip-flops. Its area is \SI{392932.89}{\um\squared}
...
Magic DRC & \num{182} boxes, \num{20} inside a macro footprint, and every
  one of the other \num{162} within \SI{0.5}{\um} of a macro
  edge \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Antenna repair trades away hold margin.}
Correction, 22 September 2026: the earlier heading ``Hold is met in the
shipping layout and lost in one variant'' misidentified the tabulated run.
The layout tabulated above is \texttt{s83ant}, with zero antenna
violations and \SI{-0.4351}{\nano\second} of worst hold slack on
\num{55} endpoints. It does not meet hold. The earlier
\texttt{s83romecc5} baseline meets hold at all three measured corners,
with \SI{+0.0296}{\nano\second} at fast, but retains two antenna
violations. Repair adds \num{60} diodes; a gentler repair setting
converges on the same failing-hold layout. Neither candidate meets all
physical acceptance conditions.
...
\paragraph{Interfaces in this historical snapshot.} SpaceWire, CAN,
