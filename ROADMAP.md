...
ports and both scrub settings. `hw/soc/formal` now contains
**95 tasks across 22 jobs**, including scrub-on/off upstream equivalence
for the `SYNPRE=1` layout profile. The older bounded/core-level results below
remain historical observations; this does not close `reg_ch0` inside Ibex.

The ROM now bounds an image to its flash slot and checks halfword entry
alignment before copying/jumping. Its range predicate has a CBMC harness.
RTL simulations are part of CI, use pinned cocotb dependencies, reject
empty/partial results, and retain logs. `make help` lists the entry points.

The physical timing/DRC experiments, pad ring and DFT, additional interfaces,
mission-specific network scaling, owner funding decisions, fabrication,
and radiation qualification remain open. They are not implied by these
software and RTL verification results. See the explicit work inventory in
...
Actions is refusing to start jobs on this account -- *(the exact wording is the owner's account status and is not published: it names a billing condition on the account, not a fault in this repository)* -- and the Tiny Tapeout GDS action is the
...
[their measured results](docs/evidence/regfile-scrub-complete-20260921.json) include six reachable mutation controls.
...
[docs/100](docs/100-routed-closure.md).
