...
static int try_image(uint32_t off, uint32_t *cause, uint32_t *entry_out) {
  ...
  *entry_out = 0;
  ...
  /* Bound the source to this flash slot as well as the destination to
     RAM. Check RV32IMC halfword entry alignment before the eventual jump. */
  int geom_ok = boot_geometry_valid(load, bytes, entry, SOC_RAM_BASE,
                                    priv, BOOT_SLOT_BYTES, BOOT_IMG_BYTES);
  if (!geom_ok) {
