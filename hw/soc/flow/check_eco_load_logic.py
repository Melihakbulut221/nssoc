...
"""Prove digital ECO equations including exact antenna and disconnected clock loads.
...
See eco_logic.py for the deliberately limited equivalence scope.
"""
import argparse
import hashlib
import json
...
from eco_logic_loads import compare
...
def main():
    ...
    sources = [args.before, args.after, args.liberty, *args.macro_liberty, *args.macro_verilog,
               ...
               Path(__file__).with_name('eco_logic_loads.py')]
    ...
    result.update(status='PASS within scope', yosys_version=version, sources=expected,
                  scope=('Retained cell input equations and state definitions; verified positive buffer '
                         'contraction, exact antenna/disconnected clock loads and Boolean pin permutations. Unchanged opaque masters require '
