...
// Port-only contract for the existing transmit-only UART. The reference
...
// reception, analog pads, or a fault model. Pin-level serialization has its
...
module soc_uart_props (
  ...
  soc_uart dut (
    ...
    .pslverr_o(error), .tx_o(tx), .irq_o(irq)
  );
  ...
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin control <= 0; divider <= 0; end
      ...
      if (paddr_i == 12'h008) control <= pwdata_i & 32'hA;
