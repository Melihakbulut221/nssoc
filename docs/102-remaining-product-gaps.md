...
| P10 | DFT, MBIST and debug access | Power-on raw system-RAM MBIST is integrated in the explicit physical profile with reset isolation and core-level result outputs (docs/103). Ethernet dual-port MBIST, scan/ATPG coverage, test pads and RISC-V debug/JTAG remain absent. Vendor BIST pins stay parked because system RAM is tested through its functional A ports. Scrubbing and fault injection are not manufacturing tests. Close with implemented access, fault-model/coverage reports and functional revalidation. |
