...
| P10 | DFT, MBIST and debug access | Power-on raw system-RAM MBIST is integrated in the explicit physical profile with reset isolation and core-level result outputs (docs/103). Ethernet dual-port FIFO MBIST is now connected in RTL with sequential testing of both ports and fail-closed chip reset (docs/103); its refreshed physical verification is in progress. Scan/ATPG coverage, test pads and RISC-V debug/JTAG remain absent. Vendor BIST pins stay parked because MBIST uses the functional SRAM ports. Scrubbing and fault injection are not manufacturing tests. Close with implemented access, fault-model/coverage reports and functional revalidation. |
...
The later power-on integration in [docs/103](103-local-closure-progress.md#power-on-sram-mbist-integrated-into-soc_top) supersedes the standalone-only MBIST limitation above for system RAM. P10 stays open for the remaining test structures and physical qualification.
