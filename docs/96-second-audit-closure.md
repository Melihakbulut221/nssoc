...
| 1.8 | Close every existing product gate | OPEN. Retain all rows of docs/92, including PCIe, final timing, packaging, DFT, debug, POR and qualification. Full transistor LVS now passes for the separate [32-SRAM core candidate](evidence/sram-chip-integration-20260925.json); its uncharacterized memories do not close overall product acceptance. |
...
| 4 | Pads/ESD/package, clocks/POR, scan/MBIST/debug, integrity/AER, SRAM LVS, radiation and silicon qualification | OPEN overall. Full transistor SRAM/SoC LVS passes for the 32-SRAM core candidate; its timing, pads and remaining qualification dependencies still require separate engineering and acceptance evidence. |
...
## 25 September: complete transistor LVS of the 32-SRAM core

The [integration receipt](evidence/sram-chip-integration-20260925.json) records
fresh full-chip extraction and 130 matching circuit pairs, including all SRAM
interiors and 5,129,488 primitive devices per side, with no extraction diagnostics.
The earlier mixed-hierarchy failure is preserved; the source-only correction
has independent graph-equivalence and fault controls. Full final power grids,
digital connection proofs, 28 boot checks and 106 helper tests also pass.
[docs/100](100-routed-closure.md) records the physical repairs, source identity,
complete artifact delivery and limits. This closes that candidate's transistor
LVS comparison within items 1.8, 3.6 and 4. It does not close final timing,
SRAM characterization, foundry DRC/density on this new GDS, pads or the other
product gates, and it does not turn the older native-memory failures into passes.
