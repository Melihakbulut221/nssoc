...
### 3.1b All twenty-seven macros, run

**Open item 6 said the other twenty-six had never been run and that one
command each would show whether the count scales the way the mechanism
predicts. Run 2026-09-14, KLayout 0.30.9, deep mode, the main deck
[fact]:**

| depth | x8 | x16 | x32 | x48 | x64 |
|---|---|---|---|---|---|
| 256  | 1,768 *(c3)* | **2,148** | **2,148** | **2,148** | 2,354 |
| 512  | 1,830 *(c3)* | **2,316** | **2,316** | --- | 2,522 |
| 1024 | **2,672** | **2,672** | **2,672** | --- | 2,878 |
| 2048 | --- | --- | --- | --- | 3,538 |
| 4096 | 2,648 *(c3)* | 2,648 *(c3)* | --- | --- | --- |
| 8192 | --- | --- | 3,752 *(c4)* | --- | --- |

Two-port parts, for the same depths: `2P_256x8` 4,256, `2P_256x16`
4,320, `2P_256x32` 4,448, `2P_512x8` 4,500, `2P_512x16` 4,564,
`2P_512x32` 4,692, `2P_1024x16` 5,312, `2P_1024x32` 5,440, `2P_64x32`
4,116.

**The count is a property of the macro's STRUCTURE, not of its bit
count, and the sweep is what separates them.** Section 3.1's series ---
2,316 for one macro, 9,668 for six, 11,048 for eight --- moved array
size and instance count together and could not tell which was
responsible. This table holds depth fixed and varies width:

- **Width does not move the count at all below x64.** `1024x8`,
  `1024x16` and `1024x32` are **identical at 2,672**, across a
  four-fold difference in stored bits. `256x16`, `256x32` and `256x48`
  are identical at 2,148. `512x16` and `512x32` at 2,316.
- **x64 adds exactly 206, three times.** 256x64 is 2,354 against
  2,148; 512x64 is 2,522 against 2,316; 1024x64 is 2,878 against 2,672.
  The same increment at three different depths is not a coincidence and
  is the one place width enters.
- **Depth does move it**, monotonically: 2,148, 2,316, 2,672, 3,538 for
  256, 512, 1024, 2048 --- the row decoder, which is what grows.
- **Two ports roughly double it**, 5,312 against 2,672 at 1024x16.
- `512x16` returns **2,316**, which is `docs/12`'s and section 3.1's
  number for the one macro measured there. The series is anchored.

**What this settles, and what it does not.** It settles that the marker
population is generated by the decoder and port structure the vendor
draws, not by the memory the design asked for --- a design that halves
its word width buys no reduction whatever. It does not settle the rule
question of section 5: every one of these is the same `Sdiod`/`Cnt`
family on the same cells, and whether those are real violations is
section 4's and section 5's argument, unchanged.
