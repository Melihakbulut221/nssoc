...
| PCIe Gen3 x4 | OPEN: no complete controller/PHY instantiated. A standalone [transaction-layer register backend and DWORD packet adapter](98-pcie-transaction-backend.md) have block, native-cell and formal evidence; neither is connected to soc_top or a PHY. CRC, DLL/LTSSM and host integration remain absent. [docs/91](91-pcie-gen3-feasibility.md) records researched candidates and missing compatible physical IP. An FPGA hard-block wrapper or PIPE placeholder does not satisfy this gate. |
