...
with zero markers. Both fresh main partitions now pass all **560 categories**
with zero markers. The [exact-core physical checkpoint](evidence/sram-repaired-core-physical-20260926.json)
preserves raw results and the separately retained failed attempts. Qualified
parasitics, final timing and full-product manufacturing qualification remain open. The candidate cannot inherit the earlier
...
worst hold slack is positive in all three Liberty corners. Router DRC and
OpenROAD antenna checks pass within their scope. ECO24 still needs independent
foundry checks on its own new GDS; prior GDS passes do not transfer.
The original unfilled baseline passes independent main DRC
