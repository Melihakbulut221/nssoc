# 13 — NLnet Restack application package (held back)
...
held for scope rather than for licence: it carries the drafted text of a funding application that has not been submitted, which is
commercial material and not a research result.

The file exists here so that citations resolve. 83 documents in
this corpus refer to `docs/13` in prose, and
